								...
								<div class="details-list" :key="index" @click="goDetails(list)">
									...
									<div class="right">
										<div class="right-t">
											...
											<div class="firstRechargeMoney">
												{{$$t('firstRechargeMoney')}}:{{currencySymbol}}{{list.firstRechargeMoney}}{{currencyDenomination}}
											</div>
										</div>
										<div class="right-b">
	...
	export default {
		...
		methods: {
			onLoad(done) {
				...
				this.$post(
						`/api/MemberAgent/GetMemberAgentPage?membershipLevel=${this.membershipLevel}&pageSize=10&pageIndex=${this.pageIndex}&startDate=${startDate}&endDate=${endDate}&isDeposit=${true}`
						)
					.then(resp => {
