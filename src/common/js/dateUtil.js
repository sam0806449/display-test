 ...
 * @returns {Date}
 */
export function getZoneTime(localtime) {
	// 取本地毫秒数
...
export function dateFormatMt_8(date, format) {
	let o = {
		'M+': date.getMonth() + 1, //month
		'd+': date.getDate(), //day
		'h+': date.getHours(), //hour
...
export function getLastMonthFirst(){
	...
	const resault = new Date(date.getFullYear(), date.getMonth() - 1, 1)

	let month = resault.getMonth() +1 ;
	let day = resault.getDate();

	if (month < 10) month = '0' + month
	if (day < 10) day = '0' + day

	return resault.getFullYear() + '-' + month + '-' + day;
}

export function getLastMonthLastDay(){
	let date=new Date();

	const resault = new Date(date.getFullYear(), date.getMonth(), 0)
